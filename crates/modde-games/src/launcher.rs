...
fn detect_steam(game_dir: &Path) -> Option<String> {
    let path_str = game_dir.to_string_lossy();
...
/// DLLs that fgmod deletes at launch time. If any of these are deployed by mods,
/// we need a post-fgmod wrapper to restore them.
const FGMOD_DELETED_DLLS: &[&str] = &[
    "dxgi.dll",
    "winmm.dll",
    "nvngx.dll",
    "_nvngx.dll",
...
pub fn generate_launch_wrapper(
    ...
    // Find mod DLLs in staging that fgmod will delete
    let exe_dir = game_dir.join("bin/x64");
    let mut restore_commands = Vec::new();

    // Walk staging mods to find DLLs that go to bin/x64 and match fgmod's delete list
    ...
    for entry in std::fs::read_dir(&mods_dir).into_iter().flatten().flatten() {
        ...
        for dll_entry in std::fs::read_dir(&mod_bin_x64).into_iter().flatten().flatten() {
            ...
            if FGMOD_DELETED_DLLS.iter().any(|d| d.to_lowercase() == dll_name) {
                let src = dll_entry.path();
                let dest = exe_dir.join(&*dll_name);
                restore_commands.push((src.to_string_lossy().to_string(), dest.to_string_lossy().to_string()));
            }
        }
    }
    ...
    script.push_str("# Auto-generated by modde — restores mod DLLs that fgmod deletes\n");
    script.push_str("# Do not edit manually; re-run `modde install` or `modde deploy` to regenerate.\n\n");
    ...
    // Run game as child process so we can capture saves after exit
    script.push_str("\n\"$@\"\n");
    script.push_str("status=$?\n\n");

    // Auto-capture saves in the background after game exits
    ...
    info!(
        ...
        restores = ?restore_commands.iter().map(|(_, d)| d.clone()).collect::<Vec<_>>(),
        "generated modde launch wrapper"
    );

    println!(
        "  Generated launch wrapper at {} (restores: {})",
        wrapper_path.display(),
        restore_commands.iter().map(|(_, d)| {
            PathBuf::from(d).file_name().unwrap_or_default().to_string_lossy().to_string()
        }).collect::<Vec<_>>().join(", ")
    );
