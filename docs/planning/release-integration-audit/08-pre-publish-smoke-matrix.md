...
Before any external publish step (Codeberg release / Homebrew / COPR / AUR / winget / Scoop / Flathub / crates.io), run a matrix of smoke checks against the _actual artifacts_ in `release/`:
...
3. Smoke runner driver:
   ...
   test "$fail" -eq 0
   ```
4. Blocking-vs-warning policy: codify in a `scripts/smoke/policy.toml` or inline in the driver. Default to _all blocking_ until a specific check is downgraded; lintian/rpmlint are warning-by-default.
...
- Smoke must run on artifacts _as they will be published_, including signatures from phase 02. Order matters: build → sign → smoke → publish. Don't run smoke on unsigned binaries.
