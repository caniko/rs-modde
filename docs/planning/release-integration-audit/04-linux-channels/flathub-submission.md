...
- `flake.nix` — the `flatpak-manifest` output already exists; review whether it produces a _buildable_ manifest (with sources block) or just metadata. Likely needs source URLs+sha256s injected at release time.
