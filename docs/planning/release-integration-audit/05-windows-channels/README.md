...
Three sub-layers. 05c (Authenticode signing) is the only one that *blocks* the other two — winget and Scoop manifests can technically reference unsigned binaries, but on modern Windows that produces SmartScreen warnings that destroy install conversion. Recommend landing 05c first or in parallel and having 05a/05b reference the signed artifacts.
