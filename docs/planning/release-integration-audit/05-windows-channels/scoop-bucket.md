...
4. `bucket/modde.json` template:
   ```json
   {
     ...
     "checkver": {
       "url": "https://codeberg.org/caniko/rs-modde/releases",
       "regex": "tag/([\\d.]+)"
     },
     "autoupdate": {
       "url": "https://codeberg.org/caniko/rs-modde/releases/download/$version/modde-$version-x86_64-windows.zip"
...
- The `hash` field is matched against the _downloaded zip_, not the binaries inside — must use the zip's sha256, not the binary's.
