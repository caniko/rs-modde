# Phase 5 — Push to `origin/trunk` and monitor Forgejo Actions
...
Two commits are visible on `origin/trunk` and the Forgejo Actions
pipeline reports all CI jobs green. After this phase, the
...
4. **Locate the pipeline.** The runner is at atlas
   ...
   skill); the Codeberg Forgejo Actions UI is the canonical viewer.
   Open the project's Actions run list for the pushed commit.

5. **Monitor.** Watch the four ci.yml jobs (`lint`, `test`, `build`,
   `flake-check`) for each of their steps in order. The lint and
   test jobs are independent; build and flake-check fan out
   in parallel.
   - `lint`: `cargo fmt --all -- --check` (fast),
     `cargo clippy --workspace -- -D warnings` (minutes; fresh
     container).
   - `test`: `just coverage-ci` (slowest — llvm-cov instrumented
     build; ~10–30 minutes depending on runner CPU).
   - `build`: `cargo build --workspace --release` (~10–20 minutes),
     then `nix build .#modde .#docs .#website .#site .#flatpak-manifest`
     (cache-dependent — fast on cache hit, otherwise minutes per
     output).
   - `flake-check`: `nix flake check --keep-going --print-build-logs`
     (covers checks-only outputs; cache-dependent).
...
6. **If a step fails:**
   ...
   - Compare to Phase 4's local run. Likely deltas:
     ...
     - **Runner image mismatch.** Local runs in `nixos/nix:latest`;
       compare with the image declared in `.forgejo/workflows/ci.yml`.
       Should match.
...
- [ ] Forgejo Actions pipeline for the just-pushed commit reports
      success on all CI jobs.
...
- **CI step times out.** Forgejo Actions job timeout defaults vary;
...
- CI viewer: Codeberg Forgejo Actions UI.
