...
By the end of this phase the maintainer also knows whether the public URL will be `https://caniko.codeberg.page/rs-modde-apt/` (canonical Codeberg URL, no extra setup) or `https://modde.rs/apt/` (custom domain, requires DNS/proxy work). That decision is recorded in this phase's `## Plan` step 6 output and consumed by Phase 04's doc rewrites.
...
The choice between custom-domain and canonical URL is not deferrable. The canonical project host is `modde.rs`, so Phase 04 must use `https://modde.rs/apt/` for end-user install and security instructions once the custom-domain path is live.
...
6. Decide the public-facing URL for end users and record it inline at the bottom of the throwaway README. Either:
   ...
   - **URL choice B (custom domain)**: `https://modde.rs/apt/`. Requires the apex `modde.rs` host to proxy `/apt/` to `https://caniko.codeberg.page/rs-modde-apt/` (or to serve the same content directly). Until that lands, end users following the install docs hit a broken URL.
   Choice B is locked in by the canonical host decision. Capture the choice in the trip notes so Phase 04 has an unambiguous target.
...
- **Custom domain trap**: writing `https://modde.rs/apt/` into the install docs *before* the DNS proxy lands ships a broken install path to users. Get the proxy in place before publishing end-user apt instructions.
...
- [ ] URL choice A — `https://caniko.codeberg.page/rs-modde-apt/`
- [x] URL choice B — `https://modde.rs/apt/`

URL chosen: B — https://modde.rs/apt/
