...
5. **`docs/site/content/docs/getting-started/installation.md` → "Debian / Ubuntu (apt)" section.** Replace every occurrence of `https://caniko.codeberg.page/rs-modde-apt` with the URL chosen in Phase 01 (record from the README footer note). If choice A: `https://caniko.codeberg.page/rs-modde-apt`. If choice B: leave the URL alone and add a one-line note above the install snippet explaining that the apex is a transparent proxy to `https://caniko.codeberg.page/rs-modde-apt`.
...
- [ ] `docs/site/content/docs/getting-started/installation.md`'s apt section URL matches Phase 01's recorded URL choice exactly. No `caniko.codeberg.page/rs-modde-apt` URLs survive *unless* choice B was made AND a one-line proxy note was added.
