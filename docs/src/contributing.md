...
| CLI text and JSON snapshot tests for `modde doctor crash` | The command output is user-facing and should not drift accidentally | Add fixtures under the CLI test fixture tree once real logs are available | `nix develop . -c cargo test -p modde-cli crash` |
